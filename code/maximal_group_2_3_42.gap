...
# Finite quotient of an NEC group with signature (0;+;[-];{(2,3,42)})
# acting on a compact Riemann surface with said signature.

# Warning: the computation of StructureDescription may take a while.
...
G := f / [ a^2, b^2, c^2, (a*b)^2, (b*c)^3, (c*a)^42, ((c*a)^6*b)^2*((c*a)^8*b)^2 ];;
a := G.1;; b := G.2;; c := G.3;;
